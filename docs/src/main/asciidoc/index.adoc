...
Spring starters allow to enable behavior of your spring-boot application by adding dependencies
to the classpath.

These starters will pre-configure camunda process engine, rest-api and webapp so
they can easily be used in a standalone process application.

include::_known-issues.adoc[]

include::_gettingstarted.adoc[]
...
Applies the history configuration to the process engine. If not configured the FULL history level level will be used.
...
:leveloffset : 1
include::_camunda-enterprise.adoc[]

:leveloffset : 1
include::_properties.adoc[]

:leveloffset : 1
include::_changelog.adoc[]
